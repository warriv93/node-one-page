...
cloudinary.config({
  cloud_name: 'dgyo1x1hb',
  api_key: '272385375964931',
  api_secret: '7LLJEtL13wUT7PAyvTM3cR--D9s'
});
...
ContentType.prototype.hasIntegrations = function () {
  return [];
};

ContentType.prototype.addIntegration = function () {
  console.warn('Should implement addIntegration()');
};
...
ContentType.prototype.getRenderingName = function () {
  return this.getId();
};
...
ContentType.prototype.getExportablePages = function () {
  return [];
};
...
ContentType.prototype.getForm = function (formData) {
  ...
  //if (this.form === undefined) {
    let formSettings = this.getFormSettings(formData);
    ...
    .then(content => {
      if (!content) {
        ...
        this.assembleAdminPage(req, res, next, {
          [this.formName]: this.getForm(content),
        });
      }
    })
    .catch(next);
...
ContentType.prototype.handleDelete = function (req, res, next) {
  ...
  this.doDelete(this.deletePath === true ? req.params.id : req.params, req)
    .then(affectedRows => {
      this.emitOverPubSub('updated', {
        page: req.multipage ? req.multipage.id : null,
      });
      return affectedRows;
    })
